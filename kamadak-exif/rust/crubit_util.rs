//! This file provides macro for creating Crubit-compatible `Result` and `Option` types.
//! This is a duplicate of google3/security/audio/chord_bridge/rust/crubit_util.rs
//! with slight modifications to support custom error.
...
macro_rules! make_result_type {
        ($type:ty, $result_name:ident $( < $lt:lifetime > )?, $err:ty) => {
        #[derive(Debug)]
        pub struct $result_name $( <$lt> )? {
            inner: anyhow::Result<$type, $err>,
        }
        ...
        impl $( <$lt> )? $result_name $( <$lt> )? {
            ...
            pub fn unwrap_err_str(self) -> cc_std::std::string {
                cc_std::std::string::from(self.unwrap_err().to_string())
            }
        }

        impl $( <$lt> )? From<anyhow::Result<$type, $err>> for $result_name $( <$lt> )? {
            fn from(val: anyhow::Result<$type, $err>) -> Self {
                Self { inner: val }
            }
        }

        impl $( <$lt> )? From<$err> for $result_name $( <$lt> )? {
    ...
    ($type:ty, $result_name:ident $( < $lt:lifetime > )?) => {
        make_result_type!($type, $result_name$( < $lt > )?, anyhow::Error);
    };

    // google3/geo/ugc/streetview/util/jpeg_metadata_injector.cc
    ($type:ty, $err:ty) => {
        paste::paste! {
          make_result_type!($type, [<Result$type>], $err);
        }
    };

    ($type:ty) => {
        paste::paste! {
          make_result_type!($type, [<Result$type>], anyhow::Error);
        }
    };
}
...
mod test {
    ...
    fn test_inherent_methods_result() {
        ...
        expect_eq!(result.unwrap_err_str(), cc_std::std::string::from("error"));
        let result = ResultDemo { inner: Err(anyhow!("error")) };
