...
absl::Status BufferedZipWriter::StartFile(absl::string_view file_name,
  ...
  return FromRustResultUnit(writer_.start_file(file_name, options.options_));
}

absl::Status BufferedZipWriter::AddDirectory(
    absl::string_view file_name, const ZipWriterFileOptions& options) {
  return FromRustResultUnit(writer_.add_directory(file_name, options.options_));
}
...
absl::Status BufferedZipWriter::WriteFileContent(absl::string_view path) {
  return FromRustResultUnit(writer_.write_file_content(path));
}

absl::StatusOr<FsZipWriter> FsZipWriter::NewFromPath(absl::string_view path,
                                                     bool append) {
  ASSIGN_OR_RETURN(zip_wrapper::FsZipWriter writer,
                   FromRustFsZipWriter(
                       zip_wrapper::FsZipWriter::new_from_path(path, append)));
