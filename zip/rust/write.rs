...
mod tests {
    ...
    fn test_buffered_zip_stream_writer() {
        ...
        assert!(!writer.is_seek_possible());
        ...
        assert_eq!(archive.get_length(), 2);
        assert_eq!(archive.get_comment().as_slice(), b"archive comment");

        {
            let mut file1 = archive.get_file_by_index(0).unwrap();
            assert_eq!(file1.get_file_name().as_slice(), b"test.txt");
            assert_eq!(file1.get_file_data().unwrap().as_slice(), b"stream writer data");
        }

        {
            let file2 = archive.get_file_by_index(1).unwrap();
            assert!(file2.is_dir());
            assert_eq!(file2.get_file_name().as_slice(), b"mydir/");
        }
    }
    ...
    fn test_free_functions_stream_writer() {
        ...
        let mut archive = BufferedZipArchive::new_from_data(zip_bytes).unwrap();
        assert_eq!(archive.get_length(), 1);
        let mut file = archive.get_file_by_index(0).unwrap();
        assert_eq!(file.get_file_data().unwrap().as_slice(), b"content");
    }
    ...
    fn test_regular_writer_is_seek_possible_and_set_comment() {
        ...
        assert_eq!(archive.get_comment().as_slice(), b"comment on regular writer");
    }
}
