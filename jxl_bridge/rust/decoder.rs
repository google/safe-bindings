...
impl JxlBridgeInner {
    ...
    /// Must only be called when the decoder is in
    /// [`DecoderState::FrameHeaderDecoded`] state.
    fn frame_header_impl(&mut self) -> StatusOr<JxlBridgeFrameHeader> {
        ...
        let jxl_fh = self
            .decoder
            .frame_header()
            .ok_or_else(|| status::internal("Frame header not available from inner decoder"))?;
    ...
    fn decode_header_impl(&mut self, data: &[u8]) -> StatusOr<JxlBridgeProcessResult> {
        ...
        let result = self.decoder.process(&mut input, None, None).map_err(to_internal)?;
    ...
    fn decode_frame_impl(
        ...
        let result =
            self.decoder.process(&mut input, Some(&mut output_bufs), None).map_err(to_internal)?;
