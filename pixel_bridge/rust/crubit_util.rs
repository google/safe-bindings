...
macro_rules! make_result_type {
        ...
        impl $( <$lt> )? $result_name $( <$lt> )? {
            ...
            // Only used in some macro invocations.
            #[allow(dead_code)]
            ...
            pub fn unwrap_err(self) -> $crate::vec_u8::VecU8 {
                self.inner.unwrap_err().into()
            }
        }
...
mod test {
    ...
    make_option_type!(u32, OptionU32, PartialEq);
    ...
    fn test_inherent_methods_result() {
        ...
        // Err() case.
        let result = ResultDemo { inner: Err(String::from("error")) };
        expect_eq!(result.unwrap_or(Demo { val: 43 }), Demo { val: 43 });
        let result = ResultDemo { inner: Err(String::from("error")) };
        expect_eq!(result.unwrap_err(), VecU8::from(String::from("error")));
    }
    ...
    fn test_inherent_methods_result_lifetime() -> Result<()> {
        ...
        // Err() case.
        let result = ResultHoldsRef { inner: Err(String::from("error")) };
    ...
    fn test_inherent_methods_option() {
        ...
        // Some() case.
        let option = OptionU32 { inner: Some(32) };
        expect_true!(option.is_some());
        expect_false!(option.is_none());
        expect_eq!(option.unwrap(), 32);

        let option = OptionU32::from_some(32);
        expect_true!(option.is_some());
        expect_false!(option.is_none());
        expect_eq!(option.unwrap(), 32);

        // None case.
        let option = OptionU32 { inner: None };
        expect_false!(option.is_some());
        expect_true!(option.is_none());
        expect_eq!(option.unwrap_or(43), 43);
    }

    #[gtest]
    fn test_conversions_option() {
        let val: CopyDemo = CopyDemo { val: 42 };
        let option = OptionCopyDemo::from(val);
        expect_true!(option.is_some());
        expect_eq!(option.unwrap(), val);

        let option = OptionCopyDemo::from(Some(val));
        expect_true!(option.is_some());
        expect_eq!(option.unwrap(), val);
    ...
    #[gtest]
    fn test_derive_traits_option() {
        // This only works if `PartialEq` was derived correctly in `OptionU32`.
        let option1 = OptionU32 { inner: Some(32) };
        let option2 = OptionU32 { inner: Some(32) };
